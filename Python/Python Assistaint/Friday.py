...
def User_Input_Handler():
    ...
    if 'according to wikipedia' in query.lower():
    ...
    elif any(phrase in query.lower() for phrase in ['open Google']):
        speak("Opening Google...")
        subprocess.run([chrome_path])
    elif any(phrase in query.lower() for phrase in ['tell me a joke']):
        print(joke)
        speak(joke)
    elif any(phrase in query.lower() for phrase in ['i did not like the joke']):
        speak("Buddy I am Program , why do expect me to have a sense of humor ?")
    ...
    elif any(phrase in query.lower() for phrase in ['open wikipedia']):
        ...
        speak("All Jedi  Are Traitors To The Republic !")
    elif any(phrase in query.lower() for phrase in ['what do you think about me']): 
        speak('You are a geek i swear to god! like who codes 6 hours a day ?')
    elif any(phrase in query.lower() for phrase in ["show me previous year jee questions"]):
        speak("N C E R T ho nahi rahi hai aur chala hai J E E kur ne")
    elif any(phrase in query.lower() for phrase in ['who is the storm that is approaching','who is the song that is approaching']):
        speak("FOOLISHNESS!"+Owner+"FOOLISHNESS,it is vergil of course you dumb neanderthal !! you think you are cool have adding this to your code ? KILL YOURSELF !!")
    elif any(phrase in query.lower() for phrase in ['turn off system']):
